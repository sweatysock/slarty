...
//	outDataV = outAudioV.slice();					// Faster way to copy venue audio to it's special output
	...
	if (echoRisk) {
		...
		let gap = 0.85/echoTest.factor;				// The factor keeps threshold high stopping feedback. 
		if (tempThresh > 0.5) tempThresh = 1.2;			// Above output of 0.5 there's no chance of getting control without muting output
		else if (tempThresh > gap) tempThresh = 0.85;		// Between gap and 0.5 there is a chance of interrupting if you shout or clap
